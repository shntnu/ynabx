...
- **Filters for spend aggregation** (skip any and the number is wrong):
  ...
  - **Split filter is question-dependent.** For category-scoped spend ("what did I spend on X") use `(parent_id IS NULL OR has_splits = FALSE)` - the `WHERE category_name = ?` clause excludes the split parent.
    For an account balance or statement match use `parent_id IS NULL` instead; the spend filter also admits split children and double-counts, inflating the balance by ~10x.
